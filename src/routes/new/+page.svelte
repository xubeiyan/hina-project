...
<div class="grow p-1 relative flex flex-col items-center">
	{#if stage == 'editForm'}
		...
		<textarea
			...
			bind:value={content}
		/>
		<div class="h-32 mb-4 flex flex-wrap gap-2 justify-center items-end w-full p-4">
			<button
			...
			<button
				class="bg-slate-200 rounded-md px-4 py-2 text-xl grow"
				on:click={() => {
					stage = 'setTimeline';
				}}
			>
	...
	{:else if stage == 'setTimeline'}
		...
		<div class="grow w-full px-8 space-y-4">
			...
			<div class="border border-yellow-400 bg-yellow-100 rounded-md">
		...
		<div class="mb-4 flex flex-wrap gap-2 justify-center items-end w-full p-4">
			<button
				class="bg-green-200 rounded-md px-4 py-2 text-xl w-full">Publish this questionnaire</button
			>
			<button
			...
			<button class="bg-blue-700 rounded-md px-4 py-2 text-xl text-slate-50 grow">
				Save all
			</button>
		</div>
	{:else if stage == 'preview'}
		<h1 class="text-lg text-center py-2">Preview Your questionnaire</h1>
		<QuestionnairePreview markdown={content} />
